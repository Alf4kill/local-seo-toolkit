...
def _collisions_html(collisions: list) -> str:
    ...
    for col in collisions:
        ...
        for m in col["members"]:
            ...
            tag = ('<b style="color:#1e7a1e">← mantém</b>' if keep
                   else '<span style="color:#b03030">→ nova keyword</span>')
            mem += (f'<div style="font-family:monospace;font-size:11px">[g{m["cluster"]}] '
...
def _link_plan_html(c: dict) -> str:
    ...
    if p.get("missing_spoke_to_hub"):
        lis = "".join(
            f'<li><code>{H.escape(s)}</code> → <code>{hub}</code> '
            f'(âncora: <b>{anchor}</b>)</li>' for s in p["missing_spoke_to_hub"])
        items += (f'<div style="margin-top:5px"><b style="color:#b03030">Faltam links spoke → cabeça '
    ...
    if p.get("missing_hub_to_spoke"):
        lis = "".join(f'<li><code>{hub}</code> → <code>{H.escape(s)}</code></li>'
                      for s in p["missing_hub_to_spoke"])
        items += (f'<div style="margin-top:5px"><b style="color:#b06000">Faltam links cabeça → spoke '
...
def _linkgraph_html(lg: dict) -> str:
    ...
    if cls is not None:
        ...
        chip = ('<span style="padding:3px 9px;border-radius:12px;font-size:12px;font-weight:700;'
    ...
    if template_only:
        tmpl_block = (
            ...
            + (f'<div style="font-size:11px;color:#888">+{len(template_only)-40} outras</div>'
               if len(template_only) > 40 else ''))

    money_block = ""
    if money:
        def tier_tag(slug):
            t = (cls or {}).get(slug, {}).get("tier")
            if t == "orphan":        return '<span style="color:#b03030">órfã</span>'
            if t == "template_only": return '<span style="color:#b06000">índice/widget</span>'
            return ''
        rows = "".join(
...
def print_clusters(clusters: list, backend: str, threshold: float) -> None:
    ...
    print(f"  {total_pages} páginas → {len(clusters)} grupos "
...
def _intro_html(clusters: list, has_gsc: bool, linkgraph: dict = None,
    ...
    if has_gsc:
        steps.append("Cruzamos com o <b>Google Search Console</b> (cliques, impressões e posição reais) para ver qual página de cada grupo já tem o melhor desempenho.")
    if has_diff:
        steps.append("Uma IA local sugeriu, para cada grupo, um <b>assunto, título e palavra-chave únicos por página</b> — assim elas param de competir <b>sem precisar apagar nenhuma</b>.")
    elif has_llm:
        steps.append("Uma IA local <b>leu cada grupo</b> e deu um diagnóstico (se as páginas são mesmo repetidas e o que falta nelas).")
    if has_links:
        steps.append("Mapeamos os <b>links internos</b> do site para mostrar quais páginas estão “soltas” e quais links conviria criar.")
    steps_html = "".join(f"<li>{s}</li>" for s in steps)

    gloss = [
        ("Canibalização", "Quando várias páginas do mesmo site disputam a mesma busca no Google. Elas dividem a força entre si e nenhuma fica bem posicionada."),
        ("Grupo de mesma intenção", "Conjunto de páginas que o sistema entendeu que falam da mesma coisa — as candidatas a competir entre si."),
        ("Coesão", "O quanto as páginas de um grupo são parecidas (de 0 a 1). Quanto mais alta, mais parecidas."),
    ]
    if has_gsc:
        gloss.append(("Impressões, cliques e posição", "Dados reais do Google: quantas vezes a página apareceu nas buscas, quantos cliques recebeu e em que posição média ficou."))
        gloss.append(("MANTER / página canônica", "A página mais forte do grupo (a que o Google já prefere). É a candidata natural a “cabeça” do tema."))
    if has_diff:
        gloss.append(("Diferenciação: cabeça e spokes", "Em vez de apagar páginas, damos a cada uma um ângulo único. A <b>cabeça</b> é a página principal do tema; os <b>spokes</b> são páginas-satélite com recortes específicos que apontam para a cabeça."))
    if has_gsc:
        if has_diff:
            gloss.append(("301 — e por que NÃO usamos aqui", "“301” é redirecionar (apagar) uma página para outra. Como o objetivo é <b>não perder páginas</b>, usamos a Diferenciação no lugar. A tabela de desempenho serve só para escolher a “cabeça” de cada grupo."))
        else:
            gloss.append(("301 (redirecionamento)", "Redirecionar uma página para outra: a antiga deixa de existir e a força vai para a que fica. É a forma clássica de resolver páginas duplicadas — só faça se puder perder aquela URL."))
    if has_links:
        gloss.append(("Link contextual × link de índice/widget", "Contextual = link dentro do <b>texto</b> de um artigo apontando para outro (vale muito para o Google). Índice/widget = link automático de listas, menus ou “veja também” (vale pouco)."))
        gloss.append(("Sem link contextual", "A página é alcançável (pelo índice/menu), mas <b>nenhum artigo a cita no corpo do texto</b> — então recebe pouca força interna."))
        gloss.append(("Plano de links (hub-and-spoke)", "Os links que faltam criar: cada página-satélite (spoke) deve linkar para a página principal (cabeça), usando a palavra-chave dela como texto do link."))
        gloss.append(("Money-page", "Página que já recebe tráfego real do Google — vale a pena priorizar o reforço dela."))
    if has_coll:
        gloss.append(("Colisão de palavra-chave", "Quando o plano acaba sugerindo a MESMA palavra-chave para duas páginas — precisa de um ajuste para elas não voltarem a competir."))
...
def generate_html(clusters: list, title: str, backend: str, threshold: float,
                  collisions: list = None, linkgraph: dict = None) -> str:
    multi   = [c for c in clusters if c["size"] >= 2]
    singles = [c for c in clusters if c["size"] == 1]
    total   = sum(c["size"] for c in clusters)

    def color(size):
        if size >= 5: return "#b03030"      # vermelho — duplicação grave
        if size >= 3: return "#b06000"      # laranja
...
def print_clusters_gsc(clusters: list, backend: str, threshold: float) -> None:
    ...
    for i, c in enumerate(multi, 1):
        ...
        for m in c["members_gsc"]:
            ...
            print(f"       {tag} pos {pos:>5} | {m['clicks']:>4} cli | {m['impressions']:>7,} impr  {m['slug']}")
    print("=" * 80 + "\n")


def generate_html_gsc(clusters: list, title: str, backend: str, threshold: float,
    ...
    if has_diff:
        cons_title = (f"⚡ Grupos que competem ({len(multi)}) — a tabela mostra a página "
                      "mais forte (a “cabeça”)")
        cons_note = ('Em cada grupo, a página <b>✅ MANTER</b> é a de melhor desempenho real no GSC — '
        ...
        cons_note = ('Em cada grupo, a página marcada <b>✅ MANTER</b> é a de melhor performance real no GSC '
    ...
    for i, c in enumerate(multi, 1):
        ...
        diff = ' <span style="color:#b06000">(≠ página central do cluster)</span>' if c.get("canonical_differs") else ""
        blocks += f"""
    ...
    for i, c in enumerate(diffed, 1):
        ...
        for p in d.get("paginas", []):
            ...
            if p.get("foco"):
                print(f"        foco   : {p['foco']}")
        if d.get("omitidas"):
            print(f"     +{len(d['omitidas'])} sem intenção distinta → rel=canonical (não 301): "
                  f"{', '.join(d['omitidas'])}")
    print("=" * 78 + "\n")
...
def print_link_audit(lg: dict) -> None:
    ...
    cls           = lg.get("classification")
    print("\n" + "=" * 78)
    print(f"  LINKS INTERNOS — {lg.get('n_edges', 0):,} links CONTEXTUAIS (de corpo) entre {lg.get('n_sources', 0)} páginas")
    print("  (links de índice blog.php/widget array_rand contam à parte; menu via include não entra)")
    print("=" * 78)
    ...
    if cls is not None:
        ...
        print(f"     {len(template_only):>4}  SÓ índice/widget (não-órfãs, mas sem link editorial) <- foco do hub-and-spoke")
...
def print_link_plan(planned: list) -> None:
    ...
    print("\n" + "=" * 78)
    print(f"  PLANO DE LINKS (hub-and-spoke) — {len(planned)} grupo(s), "
