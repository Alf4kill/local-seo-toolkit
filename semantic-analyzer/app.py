...
def run_analysis_thread(
    params: dict, output_queue: queue.Queue, done_callback, result_store: dict
) -> None:
    ...
    def worker() -> None:
        ...
        try:
            ...
            from analisar import AnalysisError, make_options, run_analysis
...
class MainWindow:
    ...
    def _build_config_panel(self) -> None:
        ...
        ttk.Radiobutton(
            src_row, text="Projeto Base", value="primeweb", variable=self._src_type
        ).pack(side="left")
        ttk.Radiobutton(
            src_row, text="Pasta .php/.html", value="folder", variable=self._src_type
        ).pack(side="left", padx=(14, 0))
        ttk.Radiobutton(src_row, text="URLs (.txt)", value="urls", variable=self._src_type).pack(
            side="left", padx=(14, 0)
        )
        ...
        ttk.Button(path_row, text="Procurar...", width=11, command=self._on_browse_source).grid(
        ...
        ttk.Combobox(
            ...
            width=14,
            state="readonly",
            values=("agglomerative", "threshold"),
        ...
        ttk.Button(gsc_row, text="Procurar...", width=11, command=self._on_browse_gsc).grid(
            row=0, column=1, padx=(6, 0)
        )
        ttk.Label(
            cfg,
            text="(opcional — pasta relatorios/<site> ou *_posicao.json do "
        ...
        ttk.Checkbutton(an_row, text="Julgamento LLM", variable=self._do_llm).pack(side="left")
        ttk.Checkbutton(an_row, text="Diferenciação", variable=self._do_diff).pack(
            side="left", padx=(14, 0)
        )
        ttk.Checkbutton(an_row, text="Grafo de links", variable=self._do_links).pack(
            side="left", padx=(14, 0)
        )
        ttk.Checkbutton(an_row, text="Ignorar cache", variable=self._no_cache).pack(
            side="left", padx=(14, 0)
        )
        ...
        ttk.Label(llm_row, text="(vazio = padrão)", foreground="#888888").pack(
            side="left", padx=(0, 14)
        )
        ...
        ttk.Combobox(
            llm_row,
            textvariable=self._llm_backend,
            width=12,
            state="readonly",
            values=("http", "transformers"),
        ).pack(side="left", padx=(4, 14))

        ttk.Checkbutton(llm_row, text="Descarregar ao terminar", variable=self._llm_unload).pack(
            side="left"
        )
        ...
        ttk.Label(
            cfg,
            text="(opcional — 1 linha sobre o site/nicho para o LLM, "
        ...
        ttk.Button(btn_row, text="Abrir pasta", width=12, command=self._on_open_folder).pack(
            side="left", padx=(0, 8)
        )
        ttk.Button(btn_row, text="Abrir relatório", width=14, command=self._on_open_report).pack(
            side="left", padx=(0, 8)
        )
    ...
    def _build_terminal_panel(self) -> None:
        ...
        ttk.Button(hdr, text="Limpar", width=7, command=self._clear_terminal).pack(side="right")
    ...
    def _tag_for_line(line: str) -> "str | None":
        ...
        if stripped.startswith("=") or stripped.startswith("─"):
            return "header"
        if "relatório salvo" in low or "relatorio salvo" in low:
            return "ok"
        if "[embed]" in low:
            return "embed"
        if "[llm]" in low:
    ...
    def _on_execute(self) -> None:
        ...
        if not src:
            messagebox.showwarning(
                "Fonte necessária", "Informe o caminho da fonte antes de executar."
            )
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Limiar inválido", "O limiar deve ser um número entre 0 e 1 (ex.: 0.85)."
            )
            return
        ...
        try:
            ...
            messagebox.showerror("Mínimo inválido", "Mín. chars deve ser um inteiro ≥ 0.")
            return
        ...
        run_analysis_thread(params, self._out_queue, self._on_task_done, self._result_store)
