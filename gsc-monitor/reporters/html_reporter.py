...
def _classify_range(pos) -> str:
    if pos is None: return "Sem Dados"
    if pos <= 3:    return "Top 3"
    if pos <= 10:   return "1ª Página"
    if pos <= 20:   return "2ª Página"
    if pos <= 50:   return "3ª Página"
    return "4ª+ Página"
...
def _hist_chart_data(historico: dict) -> dict:
    ...
    palette = [
        "#1F4E79", "#E65100", "#1B5E20", "#4A148C",
        "#B71C1C", "#006064", "#F57F17", "#37474F",
    ]
    datasets = []
...
<section id="kg">
  ...
  <div class="no-data">Marca "{brand}" não encontrada no Knowledge Graph do Google.</div>
    ...
    detail_html = f'<p class="kg-desc" style="color:#555;font-size:12px">{detail}</p>' if detail else ""
    url_html = f'<a href="{url}" target="_blank" style="font-size:12px">{url}</a>' if kg_result.get("url") else ""
    return f"""
<section id="kg">
  <h2>🌐 Knowledge Graph</h2>
  <div class="kg-card">
...
def _sec_orfas(orphans: list) -> str:
    if not orphans:
        return ""
    urls_html = "".join(
        f'<div class="orphan-url">🔗 {html.escape(o["url"])}</div>'
...
def _sec_tracking(tracking: dict) -> str:
    ...
    for r in rows:
        ...
        d = r["position_delta"]
        if d is None:   delta = '<span style="color:#888">baseline</span>'
        elif d > 0:     delta = f'<span style="color:#2d7a2d;font-weight:700">▲ +{d:.1f}</span>'
        elif d < 0:     delta = f'<span style="color:#b03030;font-weight:700">▼ {d:.1f}</span>'
        else:           delta = '<span style="color:#888">→ 0</span>'
...
def generate_dashboard(
    ...
    hist_data = _hist_chart_data(historico_posicao) \
        if historico_posicao and len(historico_posicao.get("snapshots", [])) >= 2 else None
    ...
    tracking = (build_content_tracking(historico_posicao)
                if historico_posicao else {"rows": [], "n_content_snapshots": 0})
    ...
    if health:               nav_items.insert(0, ("saude", "🏥 Saúde"))
    if consolidated:         nav_items.append(("indexacao", "🔍 Indexação"))
    if kg_result:            nav_items.append(("kg", "🌐 KG"))
    ...
    if orphans:              nav_items.append(("orfas", "📭 Sem impressões"))
