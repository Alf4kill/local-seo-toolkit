...
class TestGscLink(unittest.TestCase):
    ...
    def test_slug_from_url(self):
        ...
        self.assertEqual(_slug_from_url("http://x.com/a/b/filhote-de-rottweiler/"), "filhote-de-rottweiler")
        self.assertEqual(_slug_from_url("https://x.com/"), "index")

    def test_enrich_escolhe_melhor_performance(self):
        clusters = [{
            "size": 3, "cohesion": 0.9, "representative": "b",
            "members": ["a", "b", "c"], "indices": [0, 1, 2],
        }]
        gsc = {
            "a": {"url": "u/a", "clicks": 2,  "impressions": 100, "position": 8.0, "ctr": 1.0, "has_data": True},
            "b": {"url": "u/b", "clicks": 0,  "impressions": 50,  "position": 5.0, "ctr": 0.0, "has_data": True},
            "c": {"url": "u/c", "clicks": 20, "impressions": 300, "position": 6.0, "ctr": 2.0, "has_data": True},
        }
    ...
    def test_enrich_membro_sem_dados(self):
        clusters = [{"size": 2, "cohesion": 0.9, "representative": "a",
                     "members": ["a", "z"], "indices": [0, 1]}]
        gsc = {"a": {"url": "u/a", "clicks": 5, "impressions": 100, "position": 4.0, "ctr": 5.0, "has_data": True}}
