...
class MainWindow:
    ...
    def _build_config_panel(self) -> None:
        ...
        ttk.Checkbutton(fmt_row, text="Excel", variable=self._fmt_excel).pack(side="left", padx=(14, 0))
        ttk.Checkbutton(fmt_row, text="TXT",   variable=self._fmt_txt  ).pack(side="left", padx=(14, 0))
        ttk.Label(fmt_row, text="  (JSON salvo automaticamente)",
        ...
        ttk.Label(opt_row, text="(vazio = todas)",
                  foreground="#888888").pack(side="left", padx=(4, 22))

        self._no_cache_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="Ignorar cache  (–no-cache)",
                        variable=self._no_cache_var).pack(side="left")

        self._do_queries_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="Canibalização",
                        variable=self._do_queries_var).pack(side="left", padx=(18, 0))

        self._do_trends_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="Tendências",
                        variable=self._do_trends_var).pack(side="left", padx=(14, 0))

        self._do_nlp_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="NLP entidades",
                        variable=self._do_nlp_var).pack(side="left", padx=(14, 0))

        self._do_content_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="Qualidade conteúdo",
                        variable=self._do_content_var).pack(side="left", padx=(14, 0))
        ...
        ttk.Entry(api_row, textvariable=self._api_key_var, width=38, show="").pack(
            side="left")
        ttk.Label(api_row, text="  (opcional — KG, Trends, NLP)",
        ...
        self._open_btn = ttk.Button(btn_row, text="Abrir pasta",
                                    command=self._on_open_folder, width=12)
        self._open_btn.pack(side="left", padx=(0, 8))

        self._dash_btn = ttk.Button(btn_row, text="Dashboard",
                                    command=self._on_open_dashboard, width=12)
    ...
    def _build_terminal_panel(self) -> None:
        ...
        ttk.Button(hdr, text="Limpar", command=self._clear_terminal,
    ...
    def _tag_for_line(line: str) -> str | None:
        ...
        if "[auth]"  in low:                        return "auth"
        if "[storage]" in low:                      return "storage"
        if "concluído" in low or "concluido" in low: return "ok"
        return None
    ...
    def _on_execute(self) -> None:
        ...
        if not site:
            messagebox.showwarning("Domínio necessário",
                                   "Informe o domínio antes de executar.")
            return

        if not self._do_index.get() and not self._do_pos.get():
            messagebox.showwarning("Nenhuma análise selecionada",
        ...
        # ── Monta parâmetros ─────────────────────────────────────────────
        formats = set()
        if self._fmt_csv.get():   formats.add("csv")
        if self._fmt_excel.get(): formats.add("excel")
        if self._fmt_txt.get():   formats.add("txt")
        ...
        if not dash_path:
            ...
            if not site:
                messagebox.showinfo("Dashboard",
                                    "Informe o domínio e execute a análise primeiro.")
                return
            domain = (
                site[len("sc-domain:"):]
