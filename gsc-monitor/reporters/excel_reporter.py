...
def _build_sheet_resumo(
    ...
    if health is not None:
        ...
        for a in health.get("component_alerts") or []:
            cell = ws.cell(
                row=r, column=1, value=f"⚠ ALERTA ({a['severity'].upper()}) — {a['message']}"
            )
    ...
    if kg_result is not None:
        ...
        if kg_result.get("found"):
            ...
            info_row(kg_start + 2, "Observação", "Marca não registrada no Google Knowledge Graph.")
...
def _build_sheet_urls(ws, rows: list[dict], domain: str, data: dict) -> None:
    ...
    t = ws["A1"]
    t.value = (
        f"URLs por Faixa de Posição  |  {domain}  |  {data['start_date']} a {data['end_date']}"
    )
    ...
    sorted_rows = sorted(rows, key=lambda r: (_classify(r["position"])[1], r["position"] or 9999))
...
def _build_sheet_trends(
    ...
    if is_gsc:
        ...
        legend = (
            "Impressões/dia do próprio site na busca (dimensão date da Search Analytics API). "
    ...
    for i, (kw, td) in enumerate(trends_data.items(), start=4):
        ...
        for col, (val, fmt) in enumerate(zip(values_list, fmts), start=1):
            ...
            cell.font = Font(size=9)
            cell.alignment = Alignment(
                vertical="center", horizontal="center" if col > 1 else "left"
            )
            if col == 3:
...
def _build_sheet_orfas(ws, orphans: list, domain: str, data: dict) -> None:
    ...
    t = ws["A1"]
    t.value = f"Páginas sem impressões  |  {domain}  |  {data['start_date']} a {data['end_date']}"
    t.font = Font(bold=True, color=HEADER_FG, size=11)
    t.fill = _fill(TITLE_BG)
    t.alignment = Alignment(horizontal="center", vertical="center")
    _row_height(ws, 1, 20)
...
def _build_sheet_canibalizacao(ws, cannibalization: list, domain: str, data: dict) -> None:
    ...
    t = ws["A1"]
    t.value = (
        f"Canibalização de Keywords  |  {domain}  |  {data['start_date']} a {data['end_date']}"
    )
    t.font = Font(bold=True, color=HEADER_FG, size=11)
    t.fill = _fill(TITLE_BG)
    t.alignment = Alignment(horizontal="center", vertical="center")
...
def _build_sheet_content_quality(ws, content_results: dict, domain: str) -> None:
    ...
    items = sorted(content_results.items(), key=lambda kv: order.get(kv[1].get("verdict", "ok"), 9))
